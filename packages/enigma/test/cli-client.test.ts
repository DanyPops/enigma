...
import { clientMain, type ClientMainDeps } from "../src/cli.ts";
...
describe("clientMain add", () => {
	it("registers via the running daemon when reachable, never touching the local registry", async () => {
		const registry = fakeRegistry({ add: () => { throw new Error("must not touch the local registry when the daemon is reachable"); } });
	...
	it("falls back to the local registry when the daemon isn't reachable at all", async () => {
		const registry = fakeRegistry({ add: (name, backends) => { expect(name).toBe("web-spider"); expect(backends).toEqual(["brave", "tavily"]); return "local-token"; } });
	...
	it("surfaces a real rejection from the running daemon directly, never falling back to a phantom local registration", async () => {
		const registry = fakeRegistry({ add: () => { throw new Error("must not touch the local registry after a real remote rejection"); } });
		const deps: ClientMainDeps = { registry, addEnigmaClient: async () => ({ ok: false, status: 409, error: 'client "web-spider" is already registered' }) };
	...
	it("falls back to the local registry on a 401 from a reachable daemon that doesn't trust this caller as admin, rather than hard-failing", async () => {
		...
		const deps: ClientMainDeps = { registry, addEnigmaClient: async () => ({ ok: false, status: 401, error: "missing or invalid bearer token" }) };
		const { logs, restore } = captureConsole();
		try {
			await clientMain(["add", "web-spider", "--backends", "brave"], deps);
		} finally {
...
describe("clientMain rotate", () => {
	it("rotates via the running daemon when reachable", async () => {
		const registry = fakeRegistry({ rotate: () => { throw new Error("must not touch the local registry when the daemon is reachable"); } });
	...
	it("a 404 from a reachable daemon falls back to the local registry (ambiguous: an old daemon predating the route, vs a genuine not-found -- the local registry resolves the real answer either way)", async () => {
		const registry = fakeRegistry({ rotate: (name) => { expect(name).toBe("web-spider"); return "local-rotated-token"; } });
		const deps: ClientMainDeps = { registry, rotateEnigmaClient: async () => ({ ok: false, status: 404, error: 'no registered client named "web-spider"' }) };
		const { logs, restore } = captureConsole();
		try {
			await clientMain(["rotate", "web-spider"], deps);
		} finally {
	...
	it("surfaces a real 409/other non-fallback rejection from the running daemon directly", async () => {
		const registry = fakeRegistry({ rotate: () => { throw new Error("must not touch the local registry after a real remote rejection"); } });
...
describe("clientMain remove", () => {
	it("removes via the running daemon when reachable", async () => {
		const registry = fakeRegistry({ remove: () => { throw new Error("must not touch the local registry when the daemon is reachable"); } });
	...
	it("falls back to the local registry when the daemon isn't reachable", async () => {
		...
		const registry = fakeRegistry({ remove: (name) => { removed = name; } });
	...
	it("a 404 from a reachable daemon falls back to the local registry", async () => {
		let removed: string | undefined;
		const registry = fakeRegistry({ remove: (name) => { removed = name; } });
		const deps: ClientMainDeps = { registry, removeEnigmaClient: async () => ({ ok: false, status: 404, error: 'no registered client named "web-spider"' }) };
	...
	it("surfaces a real non-fallback rejection from the running daemon directly", async () => {
		const registry = fakeRegistry({ remove: () => { throw new Error("must not touch the local registry after a real remote rejection"); } });
...
describe("clientMain list", () => {
	it("prefers the running daemon's own registry over the local file", async () => {
		...
		const registry = fakeRegistry({ list: () => { throw new Error("must not touch the local registry when the daemon is reachable"); } });
	...
	it("falls back to the local registry when the daemon isn't reachable", async () => {
		...
		const deps: ClientMainDeps = {
			registry,
			connectEnigmaClient: () => fakeAdminClient({
				listClients: async () => {
