...
describe("macOS Keychain master-key provider", () => {
	...
	it("distinguishes missing, locked, denied, and a bounded-timeout unavailable state from the CLI, without writing", () => {
		const cases: Array<[SecretToolResult, string]> = [
			...
			[
				{ status: 1, stdout: Buffer.alloc(0), stderr: Buffer.from("SecKeychainItemCopyContent: User interaction is not allowed.") },
				"locked",
			],
...
describe("Windows Credential Manager master-key provider", () => {
	it("persists one key for the current logon identity without crossing account identities", () => {
		...
		const otherIdentity = createWindowsCredentialManagerMasterKeyProvider(
			{ service: identity.service, account: "service-account" },
			bindings,
		);
		const key = randomBytes(32);
		first.write(key);
...
describe("real native master-key persistence", () => {
	...
	it("persists across processes on Windows; on macOS, either round-trips the real key or fails closed within budget", () => {
		...
		try {
			...
			const factory =
				process.platform === "darwin" ? "createMacosKeychainMasterKeyProvider" : "createWindowsCredentialManagerMasterKeyProvider";
