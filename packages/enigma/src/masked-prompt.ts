/**
 * Reads a secret from a real TTY with the keystrokes masked -- never echoed
 * to the terminal, so it can't land in a scrollback capture or screen
 * share the way a typed-and-visible value would. Falls back to reading one
 * line from stdin unmasked when stdin isn't a TTY (piped input, e.g.
 * `pass show brave | enigma login apikey ...`) -- there is nothing to mask
 * once the value never touched an interactive terminal in the first place.
 */
